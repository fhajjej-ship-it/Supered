...
export async function verifyNpmPackage({ root = process.cwd(), tempRoot } = {}) {
  ...
  try {
    ...
    for (const target of PACKAGE_TARGETS) {
      ...
      await writeFile(join(dest, "using-supered", "SKILL.md"), "# using-supered\n\nOutdated package verification copy.\n");
      await execFileAsync(
        "npm",
        ["exec", "--yes", "--package", tarball, "--", "supered", "doctor", "--target", target, "--dest", dest, "--fix"],
        { cwd: root, env: npmPackEnv }
      );
      await execFileAsync(
        "npm",
      ...
      await writeFile(join(dest, "using-supered", "SKILL.md"), "# using-supered\n\nOutdated upgrade verification copy.\n");
      await execFileAsync(
        "npm",
        ["exec", "--yes", "--package", tarball, "--", "supered", "upgrade", "--target", target, "--dest", dest, "--apply"],
        { cwd: root, env: { ...npmPackEnv, SUPERED_LATEST_VERSION: packageJson.version } }
      );
      await execFileAsync(
        "npm",
        ["exec", "--yes", "--package", tarball, "--", "supered", "doctor", "--target", target, "--dest", dest],
    ...
    const result = {
      ...
      repairedTargets: [...PACKAGE_TARGETS],
      upgradedTargets: [...PACKAGE_TARGETS],
